...
int main() {
    ...
    while (!glfwWindowShouldClose(window))  //glfwWindowShouldClose checks if GLFW told to close.
    {
        ...
        glm::mat4 model = glm::mat4(1.0f);  //We first create a model matrix for 3D
        model = glm::rotate(model, (float)glfwGetTime() * glm::radians(-45.0f), glm::vec3(1.0f, 0.5f, 0.0f));
        
        glm::mat4 view = glm::mat4(1.0f); //Then we create the view matrix
        // note that we're translating the scene in the reverse direction of where we want to move
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));

        glm::mat4 projection;   //Lastly, we create the projection matrix
