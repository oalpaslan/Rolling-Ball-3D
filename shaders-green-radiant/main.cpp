...
-0.5f, -0.5f, 0.0f, //Since it's a triangle, z values are 0
    ...
    while (!glfwWindowShouldClose(window))  //glfwWindowShouldClose checks if GLFW told to close.
    {
        ...
        float timeValue = glfwGetTime();                   //Retrieve the running time in secs 
        float greenValue = (sin(timeValue) / 2.0f) + 0.5f; //Then we vary the color in the range of 0.0 - 1.0 by using the sin function.
